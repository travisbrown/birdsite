...
use serde_field_attributes::{integer_str, timestamp_millis_str};
use std::borrow::Cow;
        ...
        match self {
            Self::User { user } => super::UserResult::Available(super::User {
                ...
                verified_since: user
                    .verification_info
                    .reason
                    .map(|reason| reason.verified_since_msec),
...
struct VerificationReason {
    #[serde(with = "timestamp_millis_str")]
    verified_since_msec: DateTime<Utc>,
}
